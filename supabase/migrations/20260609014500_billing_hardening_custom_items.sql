-- Migration: Hardening Billing POS Custom Items
-- Timestamp: 2026-06-09 01:45:00 UTC
...
BEGIN
  ...
  LOOP
    IF (v_item->>'variant_id') IS NULL THEN
      ...
      -- RBAC Control: Custom items can only be sold by owners
      IF v_seller_role <> 'owner'::public.user_role THEN
        PERFORM set_config('app.checkout_active', 'false', true);
        RAISE EXCEPTION 'Unauthorized: Cashiers are not permitted to checkout custom items.';
      END IF;

      -- Defensive price validation check
      IF v_db_price < 0 THEN
        PERFORM set_config('app.checkout_active', 'false', true);
        RAISE EXCEPTION 'Invalid unit price % for custom item %', v_db_price, v_sku;
      END IF;
