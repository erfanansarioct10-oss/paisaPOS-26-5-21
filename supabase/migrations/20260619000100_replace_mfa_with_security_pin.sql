-- =========================================================================
-- Replace native TOTP MFA with static Security PIN for temporary access
-- =========================================================================

-- 1. Add security_pin column to users table if not exists
ALTER TABLE public.users 
  ADD COLUMN IF NOT EXISTS security_pin text 
  CONSTRAINT check_security_pin CHECK (security_pin IS NULL OR (security_pin ~ '^[0-9]{4,6}$'));

-- 2. Drop the old function that verified step-up proofs
DROP FUNCTION IF EXISTS public.grant_privilege_delegation(
  uuid,
  uuid,
  uuid,
...
-- 3. Create the new function that verifies the security PIN directly
...
BEGIN
  ...
  -- Verify owner's security PIN
  IF v_actor.security_pin IS NULL THEN
    RETURN jsonb_build_object(
      'ok', false,
  ...
  IF p_security_pin IS NULL OR v_actor.security_pin <> p_security_pin THEN
    RETURN jsonb_build_object(
      'ok', false,
...
COMMENT ON FUNCTION public.grant_privilege_delegation(
  ...
  text,
  text
) IS 'Service-role-only transaction-safe delegation grant RPC with custom security PIN check, overlap locking, grant insert, and activity logging.';

-- 4. Revoke and grant execute permissions
