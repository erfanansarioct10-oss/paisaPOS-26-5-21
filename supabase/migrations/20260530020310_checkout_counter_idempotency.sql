-- =========================================================================
-- PaisaPOS Checkout Counter Sequencing + Idempotency
--
-- Additive hardening:
-- - keeps the legacy 9-argument checkout RPC callable
-- - adds an idempotent 10-argument RPC for server actions
-- - replaces per-store invoice COUNT(*) scans with a row-level counter
-- - adds invoice sequence metadata for future pagination/reporting/indexing
-- =========================================================================
...
-- Do not backfill historical invoices in this online migration. Rewriting a large
-- invoices table would create avoidable production rollout risk; new checkout
-- writes populate these columns and counters are seeded from invoice_number below.
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE conrelid = 'public.invoices'::regclass
      AND conname = 'invoices_sequence_metadata_contract'
  ) THEN
    ALTER TABLE public.invoices
      ADD CONSTRAINT invoices_sequence_metadata_contract CHECK (
        (fiscal_year IS NULL AND invoice_seq IS NULL)
        OR (
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_invoices_store_fiscal_year_seq_unique
  ON public.invoices(store_id, fiscal_year, invoice_seq)
...
CREATE TABLE IF NOT EXISTS public.store_invoice_counters (
  ...
  next_invoice_seq bigint NOT NULL DEFAULT 1 CHECK (next_invoice_seq > 0),
...
REVOKE ALL ON TABLE public.store_invoice_counters FROM PUBLIC, anon, authenticated;
GRANT ALL ON TABLE public.store_invoice_counters TO service_role;

WITH parsed_invoice_numbers AS (
  SELECT
    i.store_id,
    m[1]::integer AS fiscal_year,
    m[2]::bigint AS invoice_seq
  FROM public.invoices i
  CROSS JOIN LATERAL regexp_match(i.invoice_number, '^INV-([0-9]{4})-([0-9]+)$') AS m
)
...
SELECT
  ...
  fiscal_year,
  max(invoice_seq) + 1
FROM parsed_invoice_numbers
GROUP BY store_id, fiscal_year
ON CONFLICT (store_id, fiscal_year) DO UPDATE
SET
  next_invoice_seq = greatest(
    public.store_invoice_counters.next_invoice_seq,
    EXCLUDED.next_invoice_seq
  ),
  updated_at = now();
...
CREATE TABLE IF NOT EXISTS public.checkout_requests (
  ...
  created_at timestamp with time zone NOT NULL DEFAULT now(),
  updated_at timestamp with time zone NOT NULL DEFAULT now(),
  UNIQUE (store_id, user_id, idempotency_key)
);

ALTER TABLE public.checkout_requests ENABLE ROW LEVEL SECURITY;
REVOKE ALL ON TABLE public.checkout_requests FROM PUBLIC, anon, authenticated;
GRANT ALL ON TABLE public.checkout_requests TO service_role;

CREATE INDEX IF NOT EXISTS idx_checkout_requests_store_created
  ON public.checkout_requests(store_id, created_at DESC);

CREATE INDEX IF NOT EXISTS idx_checkout_requests_invoice_id
  ON public.checkout_requests(invoice_id)
...
AS $$
BEGIN
  RETURN public.create_invoice_and_deduct_stock(
    p_store_id,
    p_invoice_number,
    p_customer_name,
    p_customer_phone,
