...
function shouldFailClosed() {
  const configured = parseBooleanFlag(process.env.RATE_LIMIT_FAIL_CLOSED);
...
/** Login attempts per IP+email fingerprint: 5 per 15 minutes */
...
/** Staff invite acceptance: 5 attempts per 15 minutes per user/invite pair */
export const staffInviteAcceptLimiter = new RateLimiter(5, 15 * 60 * 1000);
...
export async function enforceRateLimit(
  ...
  if (!result.success) {
    ...
    const { writeLog } = await import("@/server/logging/logger");
