...
const baseActivityInput = {
  ...
  actionScope: "store.settings" as const,
  targetType: "store",
...
describe("activity event helper", () => {
  ...
  test("bounds oversized metadata and text fields", () => {
    const oversizedMetadata = Object.fromEntries(
      Array.from({ length: 50 }, (_, index) => [`field-${index}`, "x".repeat(1_200)]),
    );
    ...
    expect(payload.action).toHaveLength(80);
    expect(payload.action).not.toContain("\n");
  ...
  test("rejects missing required activity identifiers", () => {
    expect(() =>
      buildActivityEventPayload({
        ...baseActivityInput,
        storeId: " \n ",
      }),
    ).toThrow("Activity event store id is required.");
    ...
    ).toThrow("Activity event target type is required.");
    ...
    ).toThrow("Activity event action is required.");
  });
