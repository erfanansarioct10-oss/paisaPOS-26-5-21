...
import type { TenantContextDTO } from "@/lib/server/dal";
...
type DelegatablePrivilege = (typeof DELEGATABLE_STAFF_PRIVILEGES)[number];
type ActiveDelegationPrivilege = (typeof ACTIVE_STAFF_DELEGATION_PRIVILEGES)[number];

type PrivilegeDelegationLookupRow = {
  id: string;
  scope: DelegatablePrivilege;
...
export function evaluatePrivilege(
  ...
  if (user.role === "cashier" && delegations.length > 0) {
    if (!isDelegatablePrivilege(privilege)) {
      return {
        allowed: false,
        privilege,
        source: null,
...
async function loadCandidateDelegations(
  storeId: string,
  userId: string,
  privilege: ActiveDelegationPrivilege,
...
export async function requirePrivilege(
  ...
  if (
    ...
    const delegations = await loadCandidateDelegations(context.store.id, context.user.id, privilege as ActiveDelegationPrivilege);
