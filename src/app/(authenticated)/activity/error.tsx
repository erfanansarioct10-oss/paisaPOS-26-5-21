...
export default function ActivityError({
  ...
  useEffect(() => {
    console.error("Activity route error:", error.digest ?? error.message);
  }, [error]);
  ...
  return (
    <div className="flex min-h-[420px] items-center justify-center px-4 py-10">
      <section className="w-full max-w-md rounded-xl border border-border bg-card p-6 text-center shadow-sm">
        ...
        <h1 className="mt-4 font-outfit text-xl font-bold text-foreground">Activity could not load</h1>
        <p className="mt-2 text-sm leading-relaxed text-muted-foreground">
          The activity timeline is temporarily unavailable. Try again to reload the latest accountability events.
        </p>
        <button
          type="button"
